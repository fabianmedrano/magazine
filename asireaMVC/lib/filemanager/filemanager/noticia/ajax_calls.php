    ...
    switch ($_GET['action']) {
		...
		case 'save_img':
            ...
            if ($ftp) {
                ...
                $ftp->put($config['ftp_base_folder'].$_SESSION['upload_dir'] . $_POST['path'] . $_POST['name'], $temp, FTP_BINARY);
                ...
                file_put_contents($_SESSION['current_path'] . $_POST['path'] . $_POST['name'], $image_data);
                create_img($_SESSION['current_path'] . $_POST['path'] . $_POST['name'], $config['thumbs_base_path'].$_POST['path'].$_POST['name'], 122, 91);
                // TODO something with this function cause its blowing my mind
                new_thumbnails_creation(
                    $_SESSION['current_path'].$_POST['path'],
                    $_SESSION['current_path'].$_POST['path'].$_POST['name'],
                    $_POST['name'],
        ...
        case 'extract':
            ...
            if ($ftp) {
                ...
                $base_folder = $config['ftp_base_url'].$_SESSION['upload_dir'] . fix_dirname($_POST['path']) . "/";
            } else {
                $path = $_SESSION['current_path'] . $_POST['path'];
                $base_folder = $_SESSION['current_path'] . fix_dirname($_POST['path']) . "/";
            }
            ...
            if ($ftp) {
                unlink($path);
                $ftp->putAll($base_folder, "/".$config['ftp_base_folder'] . $_SESSION['upload_dir'] . fix_dirname($_POST['path']), FTP_BINARY);
		...
		case 'media_preview':
			...
			if($ftp){
				$preview_file = $config['ftp_base_url'].$_SESSION['upload_dir'] . $_GET['file'];
			}else{
				$preview_file = $_SESSION['current_path'] . $_GET["file"];
			}
        ...
        case 'cad_preview':
            if ($ftp) {
                $selected_file = $config['ftp_base_url'].$_SESSION['upload_dir'] . $_GET['file'];
            } else {
                $selected_file = $_SESSION['current_path'] . $_GET['file'];
            ...
            if ($ftp) {
                ...
                $url_file = $config['base_url'] . $_SESSION['upload_dir'] . str_replace($_SESSION['current_path'], '', $_GET["file"]);
            }
        ...
        case 'get_file': // preview or edit
            ...
            if ($ftp) {
                $selected_file = ($sub_action == 'preview' ? $config['ftp_base_url'].$_SESSION['upload_dir'] . $_GET['file'] : $config['ftp_base_url'].$_SESSION['upload_dir'] . $_POST['path']);
            } else {
                $selected_file = ($sub_action == 'preview' ? $_SESSION['current_path'] . $_GET['file'] : $_SESSION['current_path'] . $_POST['path']);
