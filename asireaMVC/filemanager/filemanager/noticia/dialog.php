...
if (isset($_GET['numnews'])) {
    ...
    $_SESSION['upload_dir']='/public/img/noticias/noticias/'.$numnews.'/';
    $_SESSION['current_path']='../../../public/img/noticias/noticias/'.$numnews.'/';

    echo("<script> alert( ". $numnews ."); </script>");
}
...
if ($config['show_total_size']) {
    list($sizeCurrentFolder, $fileCurrentNum, $foldersCurrentCount) = folder_info($_SESSION['current_path'], false);
}
...
if (($ftp && !$ftp->isDir($config['ftp_base_folder'] . $_SESSION['upload_dir'] . $rfm_subfolder . $subdir)) || (!$ftp && !file_exists($_SESSION['current_path'] . $rfm_subfolder . $subdir))) {
    $subdir = '';
    $rfm_subfolder = "";
}


$cur_dir		= $_SESSION['upload_dir'].$rfm_subfolder.$subdir;
...
if (!$ftp) {
    ...
    while ($cycle && $i < $max_cycles) {
        ...
        if (file_exists($_SESSION['current_path'] . $parent . "config_noticia.php")) {
            $configTemp = include $_SESSION['current_path'] . $parent . 'config_noticia.php';
...
<body>
    ...
    <input type="hidden" id="upload_dir" value="<?php echo $_SESSION['upload_dir'];?>" />
...
if ($ftp) {
    try {
        $files = $ftp->scanDir($config['ftp_base_folder'] . $_SESSION['upload_dir'] . $rfm_subfolder . $subdir);
    ...
    $files = scandir($_SESSION['current_path'] . $rfm_subfolder . $subdir);
}
...
foreach ($files as $k => $file) {
    if ($ftp) {
        ...
        if ($file != "." && $file != "..") {
            if (is_dir($_SESSION['current_path'] . $rfm_subfolder . $subdir . $file)) {
                $date = filemtime($_SESSION['current_path'] . $rfm_subfolder . $subdir . $file);
                $current_folders_number++;
                if ($config['show_folder_size']) {
                    list($size, $nfiles, $nfolders) = folder_info($_SESSION['current_path'] . $rfm_subfolder . $subdir . $file, false);
                } else {
                    $size = 0;
                }
                $file_ext = trans('Type_dir');
                ...
                $current_files_number++;
                $file_path = $_SESSION['current_path'] . $rfm_subfolder . $subdir . $file;
...
<!-- header div end -->
	...
	<div class="span12">
		<?php if( ($ftp && !$ftp->isDir($config['ftp_base_folder'].$_SESSION['upload_dir'].$rfm_subfolder.$subdir))  || (!$ftp && @opendir($_SESSION['current_path'].$rfm_subfolder.$subdir)===FALSE)){ ?>
		<br/>
        ...
        foreach ($files as $file_array) {
            ...
            if($ftp && $file!='..' && $file!=$new_name){
                //rename
                rename_folder($_SESSION['current_path'].$subdir.$file,$new_name,$ftp,$config);
                $file=$new_name;
            }
            ...
            foreach ($files as $nu=>$file_array) {
                ...
                if(!$ftp){
                    ...
                    if($file!=fix_filename($file,$config)){
                        ...
                        $file_path1=($_SESSION['current_path'].$rfm_subfolder.$subdir.$file1);
                        if(file_exists($file_path1)){
                            $i = 1;
                            $info=pathinfo($file1);
                            while(file_exists($_SESSION['current_path'].$rfm_subfolder.$subdir.$info['filename'].".[".$i."].".$info['extension'])) {
                                $i++;
                            }
                    ...
                    $file_path = $config['ftp_base_url'].$_SESSION['upload_dir'].$rfm_subfolder.$subdir.$file;
                }
                ...
                if(in_array($file_array['extension'], $config['ext_img'])){
                    ...
                    if($ftp){
                        ...
                        if($img_width<45 && $img_height<38){
                            $mini_src=$_SESSION['current_path'].$rfm_subfolder.$subdir.$file;
